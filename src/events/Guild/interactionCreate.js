    ...
    if (interaction.customId === 'drop_button') {
      try {
        ...
        if (user) {
          user.snowballs += 1;
          await user.save();
          await interaction.update({ content: `❄️ ${interaction.user} yoinked the snowball.`, components: [] });
        } else {
          const newData = new Drops({
        ...
        console.error(error);
        interaction.reply('An error occurred while handling the button interaction.');
      }
    }
